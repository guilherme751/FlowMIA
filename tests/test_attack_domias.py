...
from flowmia.attacks.domias_bnaf import DomiasAttack
...
def test_domias_attack_wrapper(tmp_path):
    X_member, X_non_member, X_synth = _random_arrays(0)
    attack = DomiasAttack(device=torch.device("cpu"))
    attack.fit(X_member, X_non_member, X_synth)
    result = attack.attack(X_member, X_non_member, test_size=10, epochs=2, save_path=str(tmp_path))
