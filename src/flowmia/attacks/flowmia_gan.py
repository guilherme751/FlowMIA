...
@register_attack("flowmia_gan")
class FlowMIAGANAttack(BaseAttack):
    """BaseAttack adapter around FlowMIA_GAN.

    Uses composition rather than inheritance: FlowMIA_GAN keeps its richer
    public API (checkpointing, plotting) available for direct use, while this
    class only adapts it to the generic fit/attack interface.
    """

    def __init__(self, device=None, **hyperparams):
        super().__init__(device=device, **hyperparams)
        self._gan: FlowMIA_GAN = None
    ...
    def fit(
        ...
        **_,
        ...
        if pre_trained_model:
            self._gan.load_model(pre_trained_model)
        else:
            self._gan.fit(epochs=epochs, fcheckpoint=fcheckpoint, save_path=save_path)
        return self
    ...
    def attack(
        ...
        **kwargs,
        ...
        results = self._gan.membership_inference(test_size=test_size, **kwargs)
        scores = np.concatenate([results["score_members"], results["score_non_members"]])
        return AttackResult(scores=scores, auc=results["auc"], extra=results)

    def plot_all(self, result: AttackResult, save_path: str, colors: dict = None) -> None:
        colors = colors or {
            "members": "#e74c3c",
            "non_members": "#3498db",
            "synthetic": "#2ecc71",
        ...
        self._gan.plot_all(results=result.extra, colors=colors, save_path=save_path)